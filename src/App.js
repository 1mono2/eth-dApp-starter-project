...
const App = () => {
  ...
  const contractAddress = "0x0729f8e19f708fb4d1a3abc000b72fa8535599c8";
  ...
  useEffect(() => {
    ...
    const onNewWave = (from, timestamp, message) => {
      ...
      setAllWaves((prevState) => [
        ...prevState, { address: from, timestamp: new Date(timestamp * 1000), message }
      ]);
    };
  ...
  */
  const wave = async () => {
    try {
      const { ethereum } = window;
      if (ethereum) {
        // ここにProvider情報が格納される。基本的にWalletの情報
        const provider = new ethers.providers.Web3Provider(ethereum);
        // Singerのインスタンスを獲得。署名付きトランザクションを送信できる。
        const signer = provider.getSigner();
        const wavePortalContract = new ethers.Contract(contractAddress, contractABI, signer);
        ...
        // コントラクトのBalanceを取得
        let contractBalance_prev = await provider.getBalance(wavePortalContract.address);
        ...
        if (contractBalance_post.lt(contractBalance_prev)) {
          console.log("user won Ether!");
        } else {
          console.log("user didn't win!");
        }
        console.log("Contract balance is now", ethers.utils.formatEther(contractBalance_post));
